...
const isLocalUploadPath = (assetPath = "") =>
  String(assetPath).startsWith("/uploads/");

const toAbsoluteUploadPath = (assetPath = "") =>
  path.join(__dirname, "..", "..", String(assetPath).replace(/^\/+/, ""));

const collectQuestionImagePaths = async (connection, practiceExamId) => {
  ...
  return rows
    .map((row) => String(row.imgurl || "").trim())
    .filter(Boolean);
};

const removeLocalUploadFiles = async (assetPaths = []) => {
  const uniqueLocalPaths = [...new Set(assetPaths.filter(isLocalUploadPath))];

  await Promise.all(
    uniqueLocalPaths.map(async (assetPath) => {
      try {
        await fs.unlink(toAbsoluteUploadPath(assetPath));
      } catch (error) {
        if (error.code !== "ENOENT") {
          console.error(`Failed to remove uploaded file: ${assetPath}`, error);
        }
      }
    }),
  );
...
const insertExamQuestions = async (connection, practiceExamId, normalizedExam) => {
  for (let index = 0; index < normalizedExam.questions.length; index += 1) {
    ...
    await connection.query(
      `
        ...
        VALUES (?, ?, ?)
      `,
      [practiceExamId, questionId, index + 1],
    );
  }
};
...
exports.createExam = async (examData) => {
  ...
  try {
    ...
    await insertExamQuestions(connection, examId, normalizedExam);
...
exports.updateExam = async (practice_exam_id, examData) => {
  ...
  try {
    ...
    existingImagePaths = await collectQuestionImagePaths(
      connection,
      practice_exam_id,
    );
    await deleteExamDependencies(connection, practice_exam_id);

    await insertExamQuestions(connection, practice_exam_id, normalizedExam);

    await connection.commit();
    await removeLocalUploadFiles(
      existingImagePaths.filter(
        (assetPath) =>
          !normalizedExam.questions.some(
            (question) => String(question.imgUrl || "") === assetPath,
          ),
      ),
    );
...
exports.deleteExam = async (practice_exam_id) => {
  ...
  try {
    ...
    await connection.commit();
    await removeLocalUploadFiles(existingImagePaths);
    return result;
  } catch (error) {
    await connection.rollback();
    throw error;
