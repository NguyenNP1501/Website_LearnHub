    ...
    <article className="exam-card" key={exam.id}>
      <div className="client-tag-list">
        ...
        <span className="client-tag">
          {exam.grade ? `Lớp ${exam.grade}` : "Chưa có khối"}
        </span>
      </div>
      ...
      {!loading && !error && isSearching && (
        <section className="client-section">
          <div className="client-section__header">
            ...
            <p className="client-section__hint">
              {filteredExams.length} kết quả phù hợp với từ khóa "{keyword.trim()}"
            </p>
          </div>

          {filteredExams.length === 0 ? (
            <div className="empty-state">
              Không tìm thấy kết quả phù hợp. Hãy thử với từ khóa khác.
            </div>
          ) : (
