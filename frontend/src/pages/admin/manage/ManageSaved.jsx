...
function ManageSaved() {
  ...
  const savedExams = exams.filter(
    (item) => item.saved === true && item.deleted === false,
  );
  ...
  const handleExport = async (exam) => {
    try {
      ...
      setNotice(buildNotice("success", "Đề thi đã được xuất bản thành công."));
      await refreshExams();
    } catch (error) {
      setNotice(buildNotice("error", error.message || "Không thể xuất bản đề thi."));
    }
  };
