...
function ManageDeleted() {
  ...
  const handleDelete = async (exam) => {
    try {
      ...
      setNotice(buildNotice("success", "Đã xóa thành công."));
      await refreshExams();
    } catch (error) {
      setNotice(buildNotice("error", error.message || "Không thể xoá được."));
    }
  };
  ...
  const renderCard = (exam) => (
    <div className="list__item" key={exam.id}>
      ...
      <div className="btn">
        ...
        <button className="btn__del" onClick={() => handleDelete(exam)}>
          Xoá vĩnh viễn
        </button>
      </div>
    </div>
